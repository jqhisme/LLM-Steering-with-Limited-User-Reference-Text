...
class FASG_Model(nn.Module):
    ...
    def forward(self, encoded_input,prompt,return_type = "str"):
        ...
        def steering_features(value, hook,steering_vector = steering_vector,steering_weights = 4):
            ...
            # only take the top 5 important feature to steer
            _, top_indices = torch.topk(delta_activation, k=5, dim=-1)
            mask = torch.zeros_like(delta_activation, dtype=torch.bool)
            mask.scatter_(-1, top_indices, True)
            filtered_activation = delta_activation * mask


            steered_activation = filtered_activation.unsqueeze(1)*steering_weights + value
...
class FASG_Loss(nn.Module):
    ...
    def forward(self, reference_text,steered_text,baseline_text,temp= 1):
        ...
        margin = 0.4
        triplet_loss = torch.max(sim_positive-sim_negative+margin,0)[0] # torch max returns (max, max_indices)
...
for epoch in range(epochs):
    ...
    with open("training_log_ranksteer.txt","a") as f:
    ...
    for index in tqdm(range(0,len(trainset),batch_size)):
        ...
        torch.save(model.state_dict(),f"model_ckpts/fasg_model_epoch{epoch}_ranksteer.pth")
